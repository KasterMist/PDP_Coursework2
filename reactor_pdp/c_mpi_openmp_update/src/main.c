...
static void initialiseNeutrons(struct simulation_configuration_struct*, struct ProcConfig);
static double getControlRodLoweredToLevel(struct simulation_configuration_struct*, int, int);
static void writeReactorState(struct simulation_configuration_struct*, int, char*, struct ProcConfig);
...
int main(int argc, char * argv[]) {
  if (argc != 3 & argc != 4) {
    ...
    printf("You must provide two or three arguments, the configuration filename, output filename. Or you can include a seed to the code\n"); 
    return -1;
  }
  ...
  if(proc_config.rank == 0){
    ...
    printf("Model completed after %d timesteps\nTotal model time: %f secs\nTotal fissions: %ld releasing %e MeV and %e Joules\nTotal runtime: %.2f seconds\n",
      configuration.num_timesteps, (NS_AS_SEC*configuration.dt)*configuration.num_timesteps, total_num_fissions, total_mev, total_joules, getElapsedTime(start_time));
  }
...
static void generateReport(int dt, int timestep, struct simulation_configuration_struct* configuration, struct timeval start_time, struct ProcConfig proc_config) {
  unsigned long int current_active_neutrons = 0;
...
static void updateReactorCore(int dt, struct simulation_configuration_struct * configuration, struct ProcConfig proc_config) {
  ...
  // #pragma omp parallel for default(none) firstprivate(configuration, dt) shared(reactor_core)
...
static void updateFuelAssembly(int dt, struct channel_struct * channel, int channel_x, int channel_y) {
  ...
  // #pragma omp parallel for shared(channel) reduction(+:increment_num_fissions)-->risks
...
static void initialiseReactorCore(struct simulation_configuration_struct * simulation_configuration) {
  ...
  // #pragma omp parallel for default(none) firstprivate(simulation_configuration, stderr) shared(reactor_core)
  for (int i=0;i<simulation_configuration->channels_x;i++) {
...
static void writeReactorState(struct simulation_configuration_struct * configuration, int timestep, char * outfile, struct ProcConfig proc_config) {
  ...
  // free3dMatrix(total_quantities, reactor_core, configuration);
