...
bool determineAndHandleIfNeutronFuelCollision(double MeV, struct channel_struct * reactor_channel,
    int collision_prob_multiplyer, double **** modified_quantities, int channel_x, int channel_y, int fuel_pellet) {
  ...
  if (reactor_channel->contents.fuel_assembly.quantities[fuel_pellet][U235] > 0) {
    if (determineNeutronAbsorbtionByFuel(deBroglieWavelength, reactor_channel, U235, fuel_pellet, collision_prob_multiplyer)) {
      ...
      modified_quantities[channel_x][channel_y][fuel_pellet][U235] -= 1;
 ...
 * one neutron.
 **/
int fissionPu240(struct channel_struct * channel, double**** modified_quantities, int channel_x, int channel_y, int fuel_pellet) {
  ...
  if (r <0.73) {
    ...
    modified_quantities[channel_x][channel_y][fuel_pellet][Xe134] += 1;
    modified_quantities[channel_x][channel_y][fuel_pellet][Zr103] += 1;
 ...
 * 2 neutrons.
 **/
int fissionU236(struct channel_struct * channel, double**** modified_quantities, int channel_x, int channel_y, int fuel_pellet) {
  double r=generateDecimalRandomNumber();
  if (r <0.85) {
    // channel->contents.fuel_assembly.quantities[fuel_pellet][Ba141]++;
    // channel->contents.fuel_assembly.quantities[fuel_pellet][Kr92]++;
    modified_quantities[channel_x][channel_y][fuel_pellet][Ba141] += 1;
    modified_quantities[channel_x][channel_y][fuel_pellet][Kr92] += 1;
    ...
    modified_quantities[channel_x][channel_y][fuel_pellet][Xe140] += 1;
    modified_quantities[channel_x][channel_y][fuel_pellet][Sr94] += 1;
    channel->contents.fuel_assembly.quantities[fuel_pellet][U236]--;
    return 2;
  }
}
