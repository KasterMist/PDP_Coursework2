...
int main(int argc, char * argv[]) {
  if (argc != 4) {
    ...
    printf("You must provide three arguments, the configuration filename, output filename and the seed to the code\n"); 
    return -1;
  }
  time_t t;
  t = atoi(argv[3]);
...
static void updateNeutrons(int dt, struct simulation_configuration_struct * configuration, struct ProcConfig proc_config) {
  ...
  // #pragma omp parallel for default(none) shared(neutrons, neutron_index, currentNeutronIndex) private(dt, configuration, proc_config, stderr)
